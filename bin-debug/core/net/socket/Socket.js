    ...
    var Socket = (function (_super) {
        ...
        Socket.prototype.onSocketOpen = function () {
            ...
            if (this._connectFlag && this._needReconnect) {
                core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_RECONNECT);
            }
            else {
                core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_CONNECT);
            }
        ...
        Socket.prototype.onSocketClose = function () {
            ...
            if (this._needReconnect) {
                core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_START_RECONNECT);
                this.reconnect();
            }
            else {
                core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_CLOSE);
            }
        };
        /**
         * 服务器连接错误
        ...
        Socket.prototype.onSocketError = function () {
            if (this._needReconnect) {
                ...
                core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_NOCONNECT);
            }
            this._isConnecting = false;
        };
        /**
        ...
        Socket.prototype.connect = function () {
            if (core.App.DeviceUtils.IsHtml5) {
                if (!window["WebSocket"]) {
                    core.Log.error("不支持WebSocket");
                    return;
                }
            }
            this._socket = new egret.WebSocket();
            if (this._msg instanceof core.ByteArrayMsg) {
                this._socket.type = egret.WebSocket.TYPE_BINARY;
            }
            core.Log.debug("WebSocket: " + this._host + ":" + this._port);
            this.addEvents();
        ...
        Socket.prototype.debugInfo = function (str) {
            core.App.MessageCenter.dispatch(core.SocketConst.SOCKET_DEBUG_INFO, str);
        };
        return Socket;
    }(core.Singleton));
    core.Socket = Socket;
