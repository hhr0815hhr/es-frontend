...
(function (core) {
    ...
    var ByteArrayMsgByProtobuf = (function (_super) {
        ...
        function ByteArrayMsgByProtobuf() {
            ...
            _this.msgClass = {};
            _this.protoConfig = core.App.ProtoConfig;
        ...
        ByteArrayMsgByProtobuf.prototype.decode = function (msg) {
            ...
            if (msg.bytesAvailable >= len) {
                ...
                core.App.DebugUtils.start("Protobuf Decode");
                obj.body = this.getMsgClass(obj.key).decode(bytes.buffer);
                core.App.DebugUtils.stop("Protobuf Decode");
                core.Log.debug("收到数据：", "[" + msgID + " " + obj.key + "]", obj.body);
                return obj;
            }
            return null;
        };
        ...
        ByteArrayMsgByProtobuf.prototype.encode = function (msg) {
            ...
            core.App.DebugUtils.start("Protobuf Encode");
            var bodyBytes = new egret.ByteArray(msgBuffer);
            core.App.DebugUtils.stop("Protobuf Encode");
            core.Log.debug("发送数据：", "[" + msgID + " " + msg.key + "]", msg.body);
